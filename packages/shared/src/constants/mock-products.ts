...
const IWMS = "IWMS" as ProductCategory;
    ...
    specGroups: [
      {
        ...
        items: [
          S("Deployment", "部署方式", "SaaS 云部署 / 本地私有化部署"),
          S("Concurrent Devices", "并发设备数", "500+"),
          S("Multi-Warehouse", "多仓管理", "100+ 仓，集团化多组织"),
          S("Languages", "界面语言", "EN / ZH（可扩展多语言）"),
        ],
      },
      {
        ...
        items: [
          ...
          S("Upstream", "上游对接", "ERP / OMS / TMS / 电商平台"),
          S("Downstream", "下游对接", "WCS / AGV / AS-RS / 输送分拣"),
          S("Data Sync", "数据同步", "实时 WebSocket + 消息队列"),
        ],
      },
      {
        group: L("Security", "数据安全"),
        items: [
          S("Access Control", "权限管理", "RBAC 角色权限 + 数据权限"),
          S("Encryption", "传输加密", "TLS 1.3"),
          S("Audit", "审计", "全量操作审计日志"),
      ...
      },
      {
        ...
        items: [
          S("Dashboards", "看板", "实时库存 / 订单 / 设备看板"),
          S("Reports", "报表", "50+ 标准报表模板"),
          S("Forecasting", "预测", "AI 销量预测与补货建议"),
          S("Alerts", "告警", "库存预警多渠道推送"),
        ],
      },
    ],
    features: [
